// Edits to lines 94,98,101,124,125 as described here: https://github.com/xguaita/Leaflet.MapCenterCoord/issues/14
L.Control.MapCenterCoord = L.Control.extend({
  ...
  _getLatLngCoord: function (center) {
    ...
    return L.Util.template(this.options.template, {
      x: (!this.options.latlngDesignators ? lng : (center.lng_neg ? lng.substr(1) : lng)) + (this.options.latlngDesignators ? (center.lng_neg ? ' W' : ' E') : ''),
      y: (!this.options.latlngDesignators ? lat : (center.lat_neg ? lat.substr(1) : lat)) + (this.options.latlngDesignators ? (center.lat_neg ? ' S' : ' N') : '')
    });
  },
